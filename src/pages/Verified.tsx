...
type PreviewState = { imageSrc: string };
...
export default function PreviewPage() {
  ...
  const { tutorialId: paramTutorialId, stepNumber: paramStepNumber } = useParams<{
    tutorialId: string;
    stepNumber: string;
  }>();

  const tutorialId = paramTutorialId ?? "treehacks";
  const stepId = Math.max(1, Number(paramStepNumber ?? "1"));
  ...
  const handleBack = () => {
    navigate(`/tutorial/${tutorialId}/step/${stepId}/camera`);
  };
  ...
  const handleRetake = () => navigate(`/tutorial/${tutorialId}/step/${stepId}/camera`);
  ...
  const handleContinueToNextStep = () => {
    navigate(`/tutorial/${tutorialId}/step/${stepId}/verified`, {
      state: { totalSteps, nextStepNumber: stepId + 1 },
    });
  };
